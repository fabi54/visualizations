...
public class TestVisualizationEvolutionPolynomialOneVsMore {
	

	protected static String PATH = "C:\\Users\\janf\\Documents\\Skola\\Dip\\Project\\Data\\Results\\Tests\\Evolution_Polynomial_One_Many\\";
	
	protected static DataSource data = new RandomArtificialData();
	
	public static final Logger logger = Logger.getLogger("Test Visualization Evolution Poly");
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		ModelSource[] msa = m.getModels(data);
		for (int q = 0; q < msa.length; q++) {
			bw2.write(msa[q].getName());
			bw2.newLine();
		}
		bw2.newLine();
		
		evolution.setModeller(m);
		saveVisualizations(evolution, path + "10_");
		...
		m = new Modeller() {
			...
			public ModelSource[] getModels(DataSource data) {
				if (templates == null) {
					...
					PolynomialModelConfig cfg = new PolynomialModelConfig();
					cfg.setMaxDegree(3);
					BoostingRTModelConfig bcfg = new BoostingRTModelConfig();
					bcfg.addBaseModelCfg(cfg);
					for (int i = 0; i < templates.length; i++) {
						templates[i] = bcfg;
					}
				}
	...
	protected static void saveVisualizations(VisualizationEvolution evolution, String pref) throws IOException {	
		...
		for (int j = 0; j < vis.length; j++) {
			...
			ImageIO.write(img, "png",new File(pref + n + ".png"));
		}
	}
}
