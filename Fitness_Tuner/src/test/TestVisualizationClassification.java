...
public class TestVisualizationClassification {
			...
			public DataSource getDataSource(int index) {
				return new DataSource() {
					...
					public double[][] getOutputDataVectors() {
						if (out == null) {
							out = new double[1000][3];
							double[][] in = getInputDataVectors();
							for (int i = 0; i < in.length; i++) {
								out[i][0] = (in[i][0] < 0.5) ? 1 : 0;
								out[i][1] = (in[i][0] >= 0.5 && in[i][1] < 0.5) ? 1 : 0;
								out[i][2] = (in[i][0] >= 0.5 && in[i][1] >= 0.5) ? 1 : 0;
							}
						}
						return out;
					}
					...
					public double[][] getInputDataVectors() {
						if (in == null) {
							double[] x = StatisticSample.randomUniform(1000, 0, 1);
							double[] y = StatisticSample.randomUniform(1000, 0, 1);
							in = new double[1000][2];
			...
			public ModelSource getModelSource(int index) {
				return new ModelSource() {
					...
					public double[][] getModelResponses(double[][] inputs) {
						double[][] res = new double[inputs.length][3];
						for (int i = 0; i < inputs.length; i++) {
							res[i][0] = 1 - inputs[i][0];
							res[i][1] = 1 - inputs[i][1];
							res[i][2] = inputs[i][1];
						}
						return res;
					}
			...
			public Metadata getMetadata() {
				return new MetadataBase() {
					...
					public List<AttributeInfo> getOutputAttributeInfo() {
						List<AttributeInfo> res = new ArrayList<AttributeInfo>(3);
						res.add(new AttributeInfoBase("Class1", AttributeInfo.AttributeRole.STANDARD_OUTPUT));
						res.add(new AttributeInfoBase("Class2", AttributeInfo.AttributeRole.STANDARD_OUTPUT));
						res.add(new AttributeInfoBase("Class3", AttributeInfo.AttributeRole.STANDARD_OUTPUT));
						return res;
					}
