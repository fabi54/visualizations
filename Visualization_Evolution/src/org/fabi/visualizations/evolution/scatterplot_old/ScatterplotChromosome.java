...
public class ScatterplotChromosome extends ChromosomeBase {
	...
	public Object getPhenotype() {
		...
		cfg.setTypedProperty(ScatterplotVisualization.PROPERTY_X_AXIS_ATTRIBUTE_INDEX, indices[0]);
		cfg.setTypedProperty(ScatterplotVisualization.PROPERTY_X_AXIS_RANGE_LOWER, xLower);
		cfg.setTypedProperty(ScatterplotVisualization.PROPERTY_X_AXIS_RANGE_UPPER,
				xLower
		...
		if (yAxisInput) {
			...
			cfg.setTypedProperty(ScatterplotVisualization.PROPERTY_Y_AXIS_ATTRIBUTE_INDEX, indices[1]);
			cfg.setTypedProperty(ScatterplotVisualization.PROPERTY_Y_AXIS_RANGE_LOWER, yLower);
			cfg.setTypedProperty(ScatterplotVisualization.PROPERTY_Y_AXIS_RANGE_UPPER, yLower
					+ bounds.bounds[org.fabi.visualizations.tools.math.Arrays.RANGE][indices[1]] * lengths[1] * Math.abs(1 - starts[1]));
		}
		int inputsNr = others.length;
		List<Double> inputs = new ArrayList<Double>(inputsNr);
		for (int i = 0; i < inputsNr; i++) {
				inputs.add(others[i]);	
		}
		cfg.setTypedProperty(ScatterplotVisualization.PROPERTY_INPUTS_SETTING, inputs);
		return cfg;
	}
